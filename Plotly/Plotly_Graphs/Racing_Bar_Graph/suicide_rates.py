...
fig = go.Figure(
    ...
    layout=go.Layout(
        ...
        title=dict(text='Suicide Rates per Country: 1990',font=dict(size=28),x=.5,xanchor='center'),
        # Add button
        updatemenus=[dict(
            type="buttons",
            buttons=[dict(label="&#9654;",
    ...
    frames=[
            go.Frame(
                ...
                layout=go.Layout(
                        xaxis=dict(range=[0, 60], autorange=True),
                        yaxis=dict(range=[-0.5, 5.5], autorange=True,tickfont=dict(size=14)),
